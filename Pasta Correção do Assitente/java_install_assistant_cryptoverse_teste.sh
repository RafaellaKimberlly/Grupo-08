...
VERSAO=11
	...
	if [ \"$inicio\" == \"n\" ]
	then
		echo "$(tput setaf 10)[CrytoVerse]:$(tput setaf 7)  Tudo bem então, até a próxima ${name}!"
		exit
	fi
		echo "$(tput setaf 10)[CrytoVerse]:$(tput setaf 7) Vamos lá ${name}..."
		echo ""
...
if [ $? -eq 0 ]
	then
		echo "$(tput setaf 10)[CrytoVerse]:$(tput setaf 7) : Você já possui o java instalado!"
		echo ""
		echo "Nosso tempo juntos chegou ao fim, muito obrigado por confiar na CryptoVerse, espero ter sido útil! ${name}"
	else
		echo "$(tput setaf 10)[CrytoVerse]:$(tput setaf 7)  Ops! Não encontrei nenhuma versão do Java instalado... "
		echo ""
		echo "$(tput setaf 10)[CrytoVerse]:$(tput setaf 7) Mas fique tranquilo, posso resolver isso para você ${name}!"
		echo ""
	...
	if [ \"$inst\" == \"n\" ]
		then
			...
			echo ""
			echo "$(tput setaf 10)[CrytoVerse]:$(tput setaf 7) 1° passo - Adicionando o repositório!"
			sleep 2
			sudo add-apt-repository ppa:webupd8team/java -y
			clear
			echo "$(tput setaf 10)[CrytoVerse]:$(tput setaf 7)  Está Atualizando... Quase lá."
			echo  "$(tput setaf 10)[CrytoVerse]:$(tput setaf 7)  Verificando..."
			echo -ne '\e[1;31m #####                     (33%)\r \e[0m'
			sleep 1
			echo -ne '\e[1;31m #############             (66%)\r \e[0m'
